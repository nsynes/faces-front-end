...
class App extends React.Component {
  ...
  responseGoogle = (response) => {
    ...
    this.setState({ authorization: response.tokenObj.id_token })
  }
  ...
  showFaces = () => {
    ...
    .then((result) => {
        ...
        .then((result) => {
          ...
          const listLength = parseFloat(apiMetadata.totalFrames) / (parseFloat(apiMetadata.fps) / parseFloat(apiMetadata.checksPerSecond) ) + 1;
          ...
          for(var i = 0; i < listLength; i++) {
            ...
            time += 1/parseFloat(apiMetadata.checksPerSecond)
          }
          console.log('allFaceLocations',allFaceLocations)
          this.setState({allFaceLocations: allFaceLocations, videoMetadata: apiMetadata})
    ...
    return (
      <div className='App'>
        ...
        {this.state.videoID && this.state.authorization && <div>
          ...
          <br/>
          <br/>
          <button onClick={this.showFaces}>Show face locations</button>
        </div>}
        <br/>
        <br/>
        {this.state.videoID && <Video videoID={this.state.videoID} allFaceLocations={this.state.allFaceLocations} videoMetadata={this.state.videoMetadata} />}
      </div>
    );
  }
